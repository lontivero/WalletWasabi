		...
		internal IEnumerable<RoundStateResponse> GetStatesCollection()
		{
			...
			foreach (CoordinatorRound round in Coordinator.GetRunningRounds())
			{
				var state = new RoundStateResponse
				{
					...
					RPubKeys = Enumerable.Range(0, 10).Select( _ => round.GetNextNonce()).ToArray(),
		...
		public async Task<IActionResult> PostInputsAsync([FromBody, Required]InputsRequest request)
		{
			...
			using (await InputsLock.LockAsync())
			{
				...
				try
				{
					...
					if (round.ContainsAnyBlindedOutputScript(blindedOutputs.Select(x => x.Message)))
					{
					...
					byte[] blindedOutputScriptHashesByte = ByteHelpers.Combine(blindedOutputs.Select(x => x.Message.ToBytes()));
					...
					var alice = new Alice(inputs, networkFeeToPayAfterBaseDenomination, request.ChangeOutputAddress, acceptedBlindedOutputScripts.Select(x => x.Message));
					...
					for (int i = 0; i < acceptedBlindedOutputScripts.Count; i++)
					{
						...
						uint256 blindSignature = signer.Sign(blindedOutput.Message, round.GetNextNonceKey(blindedOutput.N));
